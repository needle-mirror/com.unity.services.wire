...
- Ungated package for use in Cloud Code 3.0.0 GA release

## [1.5.0] - 2026-08-20

### Changed

- Bumped the minimum supported Unity Editor version from 2021.3 to 6000.0.

### Fixed

- Fixed `Client.SubscribeAsync` hanging indefinitely when wire receives a terminal close code or fails the websocket open handshake.
- Fixed native crashes on PS5 (IL2CPP) caused by unguarded null dereferences of `reply.connect` during subscription recovery and the connect handshake.
- Fixed a crash in server-ping handling when the connection is disconnected concurrently, by null-checking the websocket client before sending the pong.

## [1.4.4] - 2026-05-14

### Fixed

- Fixed wire failing to connect on WebGL when using multiple UnityServices instances.
...
- Added `ResetStaticsOnLoad` with annotation `RuntimeInitializeOnLoadMethod` to `HttpUtility` so static variables are reset upon entering playmode
- Added `ResetStaticsOnLoad` with annotation `RuntimeInitializeOnLoadMethod` to `CommandID` so static variables are reset upon entering playmode
...
- Fixed Wire occasionally attempting to parse IP addresses as IPV6 on platforms that do not support IPV6.
